...
namespace Profiles.Search.Utilities
{
    public class DataIO : Framework.Utilities.DataIO
    {
        ...
        public XmlDocument CovidPersonSearchRequest(string namestring, string offset, string limit, string country)
        {
            ...
            s.Append("<SortByList></SortByList></OutputOptions></SearchOptions>");
...
//                search.Append("<SearchFilter IsExclude=\"" + isexclude + "\"  Property=\"http://profiles.catalyst.harvard.edu/ontology/prns#personInPrimaryPosition\"  Property2=\"http://profiles.catalyst.harvard.edu/ontology/prns#positionInDepartment\"   MatchType=\"Exact\">" + department + "</SearchFilter>");
        ...
        private string NameSort(string direction)
        {
            ...
            sort += "<SortBy IsDesc=\"" + (direction == "desc" ? "0" : "1") + "\" Property=\"http://xmlns.com/foaf/0.1/firstName\" />";
        ...
        public List<GenericListItem> GetCountries()
        {
            List<GenericListItem> institutions = new List<GenericListItem>();

            if (Framework.Utilities.Cache.FetchObject("GetCountries") == null)
            {
                try
                {

                    string sql = "EXEC [Profile.Data].[Organization.GetCountries]";

                    using (SqlDataReader sqldr = this.GetSQLDataReader(sql, CommandType.Text, CommandBehavior.CloseConnection, null))
                    {

                        while (sqldr.Read())
                            institutions.Add(new GenericListItem(sqldr["addressline3"].ToString(), sqldr["addressline3"].ToString()));
                    ...
                    //Defaulted this to be one hour
                    Framework.Utilities.Cache.SetWithTimeout("addressline3", institutions, 3600);
            ...
            {
                institutions = (List<GenericListItem>)Framework.Utilities.Cache.FetchObject("addressline3");
